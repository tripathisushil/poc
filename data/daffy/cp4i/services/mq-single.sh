...
{
  if [ "${1}" = "delete" ]; then
     printHeaderMessage "Cloud Pak for Integration Service - MQ Single Instance" ${RED_TEXT}
  else
     printHeaderMessage "Cloud Pak for Integration Service - MQ Single Instance"
  fi
  if [ "${CP4I_ENABLE_SERVICE_MQSINGLE}" == "true" ]; then
      if [ ${1} == "apply" ]; then
               ...
               MQ_OPERATOR_SUBSCRIPTION_READY=`oc get csv -n ${CP4I_NAMESPACE} | grep  ibm-mq 2> /dev/null | grep -c Succeeded`
                  if [ ${MQ_OPERATOR_SUBSCRIPTION_READY} == "1" ]  ;then
                     ...
                     if [ ${CP4I_ENABLE_SERVICE_TRACING} == "true" ]; then
                        ...
                        oc apply -f ${TEMP_DIR}/${PRODUCT_SHORT_NAME}/customresource/mq-single.yaml
                        echo ""
                        ...
                        echo "            ${DATA_DIR}/${PROJECT_NAME}/${PRODUCT_SHORT_NAME}/service.sh ${ENV_FILE_NAME} --MQSingleStatus"
                        echo ""
                  fi
                  ...
                  if [ ${ODSERVICE_BINDING} == "1" ] ;then
                     ...
                     oc apply -f ${TEMP_DIR}/${PRODUCT_SHORT_NAME}/customresource/mqodtracing.yaml -n ${CP4I_NAMESPACE}
                  fi
      fi
  else
      if [ ${1} == "delete" ]; then
         echo "Removing Operand Request"
         oc delete operandrequest mq0-ibm-mq -n ${CP4I_NAMESPACE} 2> /dev/null
         echo ""
         echo "Removing Custom Resource"
         oc delete --grace-period=0 --force --ignore-not-found=true --include-uninitialized=true --timeout=30s -f ${TEMP_DIR}/${PRODUCT_SHORT_NAME}/customresource/mq-single.yaml 2> /dev/null
         oc delete crd queuemanagers.mq.ibm.com 2> /dev/null
         echo ""
         echo "Removing Service"
         oc delete QueueManager mq0 -n ${CP4I_NAMESPACE} 2> /dev/null
         echo ""
      fi
  fi
  echo ""
...
cp4iServiceMQSingleStatus()
{
  ...
  local MQ_VERSION=`oc get QueueManager mq0 --namespace=${CP4I_NAMESPACE} -o jsonpath='{.spec.version}' 2> /dev/null`
  if [ -z ${MQ_VERSION} ]; then
    MQ_VERSION=""
  fi
  #Status good locic, need to test -o jsonpath='{.status.message}'
  local MQ_STATUS=`oc get QueueManager mq0 -n ${CP4I_NAMESPACE} -o jsonpath='{.status.phase}' 2> /dev/null`
  if [ -z ${MQ_STATUS} ]; then
    MQ_STATUS="Not Found"
  fi
  echo "MQ Single Instance                      :  ${MQ_STATUS} - ${MQ_VERSION}"
  echo "Pods:  "
  oc get pods -A -l 'app.kubernetes.io/instance=mq0'
  echo ""
